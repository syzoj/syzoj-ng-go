...
type LockManager interface {
	...
	WithLockShared(ctx context.Context, id string, wait bool, handler func(context.Context, SharedLock) error) error
	// Locks the specified resource in exclusive mode.
	// The inner context will be cancelled when the lock expires.
	// The handler may not be called at all, in which case an error will be returned.
	WithLockExclusive(ctx context.Context, id string, wait bool, handler func(context.Context, ExclusiveLock) error) error
}
